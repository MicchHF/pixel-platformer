...
export const LevelEditor: React.FC<LevelEditorProps> = ({
  ...
  const [levelCols, setLevelCols] = useState<number>(initialLevel?.cols || 18);
  const [levelRows, setLevelRows] = useState<number>(initialLevel?.rows || 18);
  ...
  const handleSelectLevelToEdit = (lvlId: number) => {
    ...
    if (found) {
      ...
      setLevelCols(found.cols || 18);
      setLevelRows(found.rows || 18);
