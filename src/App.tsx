import React, { useState, useEffect, useCallback, useMemo } from 'react';
import { HUD } from './components/HUD';
import { GameCanvas } from './components/GameCanvas';
import { LevelSelectModal } from './components/LevelSelectModal';
import { SettingsModal } from './components/SettingsModal';
import { LevelEditor } from './components/LevelEditor';
import { EmbedDrawer } from './components/EmbedDrawer';
import { VictoryModal } from './components/VictoryModal';
import { LeaderboardModal } from './components/LeaderboardModal';
import { NavigationBar } from './components/NavigationBar';
import { LEVELS } from './game/levels';
import { THEMES } from './game/themes';
import { sounds } from './audio/soundManager';
import { GameEngineState, initLevelState } from './game/physics';
import { LevelData, LevelRecord, ThemeName } from './types/game';
import { initTelegram, haptics, shareScoreToTelegram, TelegramUserData } from './utils/telegram';
import { submitScoreToLeaderboard, batchSyncRecords } from './services/leaderboardApi';
import { fetchServerBaseLevels } from './services/levelsApi';
import { formatHundredths } from './utils/time';
import { Swords, X as CloseIcon } from 'lucide-react';

const STORAGE_KEY = 'hardcore_pixel_platformer_v1';
const LIFETIME_DEATHS_KEY = 'hardcore_pixel_platformer_lifetime_deaths';
const CUSTOM_LEVELS_STORAGE_KEY = 'hardcore_pixel_platformer_custom_levels_v6_base_sync';
const PLAYER_NAME_KEY = 'pixel_platformer_player_name';
const CREATOR_MODE_KEY = 'hardcore_platformer_creator_mode';

function loadStoredLevels(): LevelData[] {
  try {
    if (typeof window === 'undefined') return LEVELS;
    
    // Check if user has custom created levels
    const stored = localStorage.getItem(CUSTOM_LEVELS_STORAGE_KEY);
    if (!stored) {
      try {
        localStorage.setItem(CUSTOM_LEVELS_STORAGE_KEY, JSON.stringify(LEVELS));
      } catch {}
      return LEVELS;
    }

    const parsed: LevelData[] = JSON.parse(stored);
    if (!Array.isArray(parsed) || parsed.length === 0) return LEVELS;

    // Filter only genuine custom user levels (id > 15)
    const customOnly = parsed.filter((lvl) => lvl.id > 15);
    
    // Always guarantee the official 15 campaign levels are fresh base maps
    const merged = [...LEVELS, ...customOnly];
    return merged;
  } catch {
    return LEVELS;
  }
}

export default function App() {
  // Telegram User & Initialization
  const [tgUser] = useState<TelegramUserData>(() => initTelegram());
  const [playerName, setPlayerName] = useState<string>(() => {
    if (tgUser.firstName) return tgUser.firstName;
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem(PLAYER_NAME_KEY);
        if (saved) return saved;
      } catch {}
    }
    return 'Бегущий';
  });

  const handleUpdatePlayerName = useCallback((name: string) => {
    const clean = name.trim().slice(0, 24) || 'Бегущий';
    setPlayerName(clean);
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(PLAYER_NAME_KEY, clean);
      } catch {}
    }
  }, []);

  // Admin / Creator Mode (Allows modifying base campaign levels)
  const [isCreatorMode, setIsCreatorMode] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      try {
        return localStorage.getItem(CREATOR_MODE_KEY) === 'true';
      } catch {}
    }
    return false;
  });

  const handleToggleCreatorMode = useCallback((enabled: boolean) => {
    setIsCreatorMode(enabled);
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(CREATOR_MODE_KEY, enabled ? 'true' : 'false');
      } catch {}
    }
  }, []);

  // Parse URL query parameters
  const urlParams = useMemo(() => {
    if (typeof window === 'undefined') return new URLSearchParams();
    return new URLSearchParams(window.location.search);
  }, []);

  const initialEmbed = urlParams.get('embed') === 'true' || urlParams.get('embed') === '1';
  const initialCompact = urlParams.get('compact') === 'true' || initialEmbed;
  const initialLevelParam = parseInt(urlParams.get('level') || '1', 10);
  const initialThemeParam = (urlParams.get('theme') as ThemeName) || 'neon';
  const initialMutedParam = urlParams.get('muted') === 'true' || urlParams.get('mute') === '1';
  
  // Friend challenge banner from Telegram URL query
  const challengeTimeParam = urlParams.get('challengeTime');
  const [challengeBanner, setChallengeBanner] = useState<string | null>(() => {
    return challengeTimeParam ? challengeTimeParam : null;
  });

  // Game Levels (Persistent with localStorage support)
  const [levels, setLevels] = useState<LevelData[]>(() => loadStoredLevels());
  const [currentLevelIndex, setCurrentLevelIndex] = useState<number>(() => {
    const validId = isNaN(initialLevelParam) ? 1 : Math.max(1, Math.min(LEVELS.length, initialLevelParam));
    const idx = LEVELS.findIndex((l) => l.id === validId);
    return idx !== -1 ? idx : 0;
  });

  const currentLevel = levels[currentLevelIndex] || levels[0];

  // Appearance & Audio Settings
  const [themeName, setThemeName] = useState<ThemeName>(() => {
    return THEMES[initialThemeParam] ? initialThemeParam : 'neon';
  });
  const currentTheme = THEMES[themeName] || THEMES.neon;

  const [scanlines, setScanlines] = useState<boolean>(false);
  const [screenShakeEnabled, setScreenShakeEnabled] = useState<boolean>(true);
  const [touchMode, setTouchMode] = useState<'auto' | 'always' | 'hidden'>('auto');
  const [isCompact] = useState<boolean>(initialCompact);

  // Audio State
  const [isMuted, setIsMuted] = useState<boolean>(initialMutedParam);
  const [sfxVolume, setSfxVolume] = useState<number>(0.6);
  const [bgmVolume, setBgmVolume] = useState<number>(0.3);
  const [isBgmPlaying, setIsBgmPlaying] = useState<boolean>(false);

  useEffect(() => {
    sounds.setMuted(isMuted);
    sounds.setSfxVolume(sfxVolume);
    sounds.setBgmVolume(bgmVolume);
  }, [isMuted, sfxVolume, bgmVolume]);

  // Synchronize server base levels across all players/devices (especially in Telegram)
  useEffect(() => {
    fetchServerBaseLevels().then((serverLevels) => {
      if (serverLevels && Array.isArray(serverLevels) && serverLevels.length > 0) {
        setLevels((prev) => {
          const customOnly = prev.filter((l) => l.id > 15);
          const serverIds = new Set(serverLevels.map((l) => l.id));
          const nonConflictingCustom = customOnly.filter((l) => !serverIds.has(l.id));
          const updated = [...serverLevels, ...nonConflictingCustom];
          if (typeof window !== 'undefined') {
            try {
              localStorage.setItem(CUSTOM_LEVELS_STORAGE_KEY, JSON.stringify(updated));
            } catch {}
          }
          return updated;
        });
      }
    });
  }, []);

  // In-Game Live HUD Tracking
  const [hudPlayer, setHudPlayer] = useState<GameEngineState['player']>(() => initLevelState(currentLevel).player);
  const [hudTime, setHudTime] = useState<number>(0);
  const [hudDeaths, setHudDeaths] = useState<number>(0);
  const [lifetimeDeaths, setLifetimeDeaths] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem(LIFETIME_DEATHS_KEY);
        if (saved) return parseInt(saved, 10) || 0;
      } catch {}
    }
    return 0;
  });

  const [isPaused, setIsPaused] = useState<boolean>(false);
  const [restartSignal, setRestartSignal] = useState<number>(0);

  // Progress & Records Storage
  const [records, setRecords] = useState<Record<number, LevelRecord>>(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem(STORAGE_KEY);
        if (saved) return JSON.parse(saved);
      } catch {}
    }
    return {};
  });

  // Modal Views
  const [showLevelSelect, setShowLevelSelect] = useState<boolean>(false);
  const [showSettings, setShowSettings] = useState<boolean>(false);
  const [showEditor, setShowEditor] = useState<boolean>(false);
  const [showEmbedDrawer, setShowEmbedDrawer] = useState<boolean>(false);
  const [showLeaderboard, setShowLeaderboard] = useState<boolean>(false);
  const [victoryData, setVictoryData] = useState<{
    level: LevelData;
    time: number;
    deaths: number;
    isNewRecord: boolean;
    rank?: number;
  } | null>(null);

  // Batch sync local records to leaderboard server on startup
  useEffect(() => {
    if (Object.keys(records).length > 0) {
      batchSyncRecords(
        playerName,
        records,
        tgUser.username,
        tgUser.id,
        tgUser.isTelegram ? 'telegram' : 'web'
      );
    }
  }, []); // Run once on startup

  // Save Progress
  const updateRecords = useCallback((levelId: number, time: number, deaths: number) => {
    setRecords((prev) => {
      const existing = prev[levelId];
      const isNewBest = !existing || time < existing.bestTime;
      const updatedRecord: LevelRecord = {
        completed: true,
        bestTime: isNewBest ? time : existing.bestTime,
        deaths: isNewBest ? deaths : Math.min(existing.deaths, deaths),
        stars: 3,
      };

      const next = { ...prev, [levelId]: updatedRecord };
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
        } catch {}
      }
      return next;
    });
  }, []);

  const incrementLifetimeDeaths = useCallback(() => {
    setLifetimeDeaths((prev) => {
      const next = prev + 1;
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(LIFETIME_DEATHS_KEY, next.toString());
        } catch {}
      }
      return next;
    });
  }, []);

  // Audio start on first interaction
  const startBgmOnce = useCallback(() => {
    if (!isBgmPlaying && !isMuted) {
      sounds.startBgm();
      setIsBgmPlaying(true);
    }
  }, [isBgmPlaying, isMuted]);

  useEffect(() => {
    const handleFirstUserGesture = () => {
      startBgmOnce();
      window.removeEventListener('click', handleFirstUserGesture);
      window.removeEventListener('keydown', handleFirstUserGesture);
      window.removeEventListener('touchstart', handleFirstUserGesture);
    };

    window.addEventListener('click', handleFirstUserGesture);
    window.addEventListener('keydown', handleFirstUserGesture);
    window.addEventListener('touchstart', handleFirstUserGesture);

    return () => {
      window.removeEventListener('click', handleFirstUserGesture);
      window.removeEventListener('keydown', handleFirstUserGesture);
      window.removeEventListener('touchstart', handleFirstUserGesture);
    };
  }, [startBgmOnce]);

  // HUD update handler memoized to avoid re-renders
  const handleHudUpdate = useCallback((p: GameEngineState['player'], t: number, d: number) => {
    setHudPlayer(p);
    setHudTime(t);
    setHudDeaths(d);
  }, []);

  // Level Complete Event Handler
  const handleLevelWin = useCallback((level: LevelData, time: number, deaths: number) => {
    haptics.success();
    const existing = records[level.id];
    const isNewRecord = !existing || time < existing.bestTime;

    updateRecords(level.id, time, deaths);

    // Auto submit to server leaderboard
    submitScoreToLeaderboard({
      levelId: level.id,
      time,
      deaths,
      playerName,
      username: tgUser.username,
      tgId: tgUser.id,
      platform: tgUser.isTelegram ? 'telegram' : 'web',
    }).then((res) => {
      setVictoryData({
        level,
        time,
        deaths,
        isNewRecord,
        rank: res?.rank,
      });
    }).catch(() => {
      setVictoryData({
        level,
        time,
        deaths,
        isNewRecord,
      });
    });
  }, [records, playerName, tgUser, updateRecords]);

  // Next Level Handler
  const handleNextLevel = () => {
    setVictoryData(null);
    if (currentLevelIndex < levels.length - 1) {
      setCurrentLevelIndex(currentLevelIndex + 1);
    } else {
      // Completed all campaign levels
      setShowLevelSelect(true);
    }
  };

  // Quick Restart Trigger
  const handleQuickRestart = useCallback(() => {
    setRestartSignal((prev) => prev + 1);
  }, []);

  // Stable Win Handler
  const handleGameWin = useCallback((time: number, deaths: number) => {
    handleLevelWin(currentLevel, time, deaths);
  }, [handleLevelWin, currentLevel]);

  // Settings Handlers
  const handleToggleMute = useCallback(() => {
    const nextMuted = !isMuted;
    setIsMuted(nextMuted);
    sounds.setMuted(nextMuted);
    if (!nextMuted && !isBgmPlaying) {
      sounds.startBgm();
      setIsBgmPlaying(true);
    }
  }, [isMuted, isBgmPlaying]);

  const handleToggleBgm = useCallback(() => {
    if (isBgmPlaying) {
      sounds.stopBgm();
      setIsBgmPlaying(false);
    } else {
      sounds.startBgm();
      setIsBgmPlaying(true);
    }
  }, [isBgmPlaying]);

  const handleSfxVolume = (vol: number) => {
    setSfxVolume(vol);
    sounds.setSfxVolume(vol);
  };

  const handleBgmVolume = (vol: number) => {
    setBgmVolume(vol);
    sounds.setBgmVolume(vol);
  };

  const handleResetProgress = () => {
    setRecords({});
    setLifetimeDeaths(0);
    if (typeof window !== 'undefined') {
      try {
        localStorage.removeItem(STORAGE_KEY);
        localStorage.removeItem(LIFETIME_DEATHS_KEY);
      } catch {}
    }
    sounds.playDeath();
  };

  // Custom Level Management (Save/Reset/Delete)
  const handleSaveLevel = (savedLevel: LevelData, asNew?: boolean) => {
    let nextLevels: LevelData[];
    if (asNew) {
      nextLevels = [...levels, savedLevel];
    } else {
      const idx = levels.findIndex((l) => l.id === savedLevel.id);
      if (idx !== -1) {
        nextLevels = [...levels];
        nextLevels[idx] = savedLevel;
      } else {
        nextLevels = [...levels, savedLevel];
      }
    }
    setLevels(nextLevels);
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(CUSTOM_LEVELS_STORAGE_KEY, JSON.stringify(nextLevels));
      } catch {}
    }
  };

  const handleResetLevel = (levelId: number) => {
    const defaultLevel = LEVELS.find((l) => l.id === levelId);
    if (defaultLevel) {
      const nextLevels = levels.map((l) => (l.id === levelId ? defaultLevel : l));
      setLevels(nextLevels);
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(CUSTOM_LEVELS_STORAGE_KEY, JSON.stringify(nextLevels));
        } catch {}
      }
    }
  };

  const handleDeleteLevel = (levelId: number) => {
    const nextLevels = levels.filter((l) => l.id !== levelId);
    setLevels(nextLevels);
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(CUSTOM_LEVELS_STORAGE_KEY, JSON.stringify(nextLevels));
      } catch {}
    }
    if (currentLevelIndex >= nextLevels.length) {
      setCurrentLevelIndex(Math.max(0, nextLevels.length - 1));
    }
  };

  // Playtest a level directly from editor
  const handlePlaytest = (levelToPlay: LevelData) => {
    let targetIdx = levels.findIndex((l) => l.id === levelToPlay.id);
    if (targetIdx === -1) {
      // Temporarily append for testing
      const newLevels = [...levels, levelToPlay];
      setLevels(newLevels);
      targetIdx = newLevels.length - 1;
    } else {
      // Update in place
      const updated = [...levels];
      updated[targetIdx] = levelToPlay;
      setLevels(updated);
    }
    setCurrentLevelIndex(targetIdx);
    setShowEditor(false);
    handleQuickRestart();
  };

  const handleSelectLevel = useCallback((target: LevelData | number) => {
    const id = typeof target === 'number' ? target : target.id;
    const idx = levels.findIndex((l) => l.id === id);
    if (idx !== -1) {
      setCurrentLevelIndex(idx);
      handleQuickRestart();
    }
    setShowLevelSelect(false);
    setShowLeaderboard(false);
  }, [levels, handleQuickRestart]);

  const activeScreen = useMemo<'game' | 'levels' | 'leaderboard' | 'editor' | 'settings'>(() => {
    if (showEditor) return 'editor';
    if (showLevelSelect) return 'levels';
    if (showLeaderboard) return 'leaderboard';
    if (showSettings) return 'settings';
    return 'game';
  }, [showEditor, showLevelSelect, showLeaderboard, showSettings]);

  const handleNavigate = useCallback((screen: 'game' | 'levels' | 'leaderboard' | 'editor' | 'settings') => {
    if (screen === 'game') {
      setShowLevelSelect(false);
      setShowLeaderboard(false);
      setShowEditor(false);
      setShowSettings(false);
      setShowEmbedDrawer(false);
      setIsPaused(false);
    } else if (screen === 'levels') {
      setShowLeaderboard(false);
      setShowEditor(false);
      setShowSettings(false);
      setShowEmbedDrawer(false);
      setShowLevelSelect((prev) => !prev);
    } else if (screen === 'leaderboard') {
      setShowLevelSelect(false);
      setShowEditor(false);
      setShowSettings(false);
      setShowEmbedDrawer(false);
      setShowLeaderboard((prev) => !prev);
    } else if (screen === 'editor') {
      setShowLevelSelect(false);
      setShowLeaderboard(false);
      setShowSettings(false);
      setShowEmbedDrawer(false);
      setShowEditor((prev) => !prev);
    } else if (screen === 'settings') {
      setShowLevelSelect(false);
      setShowLeaderboard(false);
      setShowEditor(false);
      setShowEmbedDrawer(false);
      setShowSettings((prev) => !prev);
    }
  }, []);

  return (
    <main 
      id="app-container"
      className="relative flex flex-col h-screen h-[100dvh] w-screen max-w-full overflow-hidden bg-zinc-950 select-none touch-none"
    >
      {/* SCANLINES OVERLAY */}
      {scanlines && (
        <div 
          id="scanlines-layer"
          className="pointer-events-none absolute inset-0 z-40 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.4)_50%)] bg-[length:100%_4px] opacity-30" 
        />
      )}

      {/* FRIEND CHALLENGE BANNER (Telegram link) */}
      {challengeBanner && (
        <div 
          id="challenge-banner"
          className="z-40 flex items-center justify-between px-3 py-1.5 bg-gradient-to-r from-amber-950/90 via-amber-900/90 to-amber-950/90 border-b border-amber-500/50 text-amber-200 text-xs font-mono shadow-md shrink-0"
        >
          <div className="flex items-center gap-2">
            <Swords className="w-4 h-4 text-amber-400 animate-pulse" />
            <span>
              ВЫЗОВ ОТ ДРУГА: <strong className="text-amber-300 font-bold">{challengeBanner}s</strong> на Уровне {currentLevel.id}! Побей рекорд!
            </span>
          </div>
          <button 
            type="button"
            onClick={() => setChallengeBanner(null)}
            className="p-1 hover:bg-amber-800/40 rounded text-amber-400 cursor-pointer"
          >
            <CloseIcon className="w-3.5 h-3.5" />
          </button>
        </div>
      )}

      {/* TOP HUD BAR */}
      <HUD
        level={currentLevel}
        player={hudPlayer}
        levelTime={hudTime}
        time={hudTime}
        deaths={hudDeaths}
        lifetimeDeaths={lifetimeDeaths}
        totalLifetimeDeaths={lifetimeDeaths}
        bestTime={records[currentLevel.id]?.bestTime}
        theme={currentTheme}
        isPaused={isPaused}
        isMuted={isMuted}
        isBgmPlaying={isBgmPlaying}
        onRestart={handleQuickRestart}
        onTogglePause={() => setIsPaused((prev) => !prev)}
        onToggleMute={handleToggleMute}
        onToggleBgm={handleToggleBgm}
        onOpenLevelSelect={() => setShowLevelSelect(true)}
        onOpenLevels={() => setShowLevelSelect(true)}
        onOpenSettings={() => setShowSettings(true)}
        onOpenEditor={() => setShowEditor(true)}
        onOpenEmbed={() => setShowEmbedDrawer(true)}
        onOpenLeaderboard={() => setShowLeaderboard(true)}
      />

      {/* MAIN GAME STAGE CANVAS */}
      <section 
        id="game-viewport-section"
        className="relative flex-1 w-full flex items-center justify-center min-h-0 overflow-hidden bg-zinc-950"
      >
        <GameCanvas
          level={currentLevel}
          theme={currentTheme}
          scanlines={scanlines}
          screenShakeEnabled={screenShakeEnabled}
          touchMode={touchMode}
          restartSignal={restartSignal}
          isPaused={isPaused || showLevelSelect || showSettings || showEditor || showEmbedDrawer || showLeaderboard || Boolean(victoryData)}
          onDeath={incrementLifetimeDeaths}
          onWin={handleGameWin}
          onHudUpdate={handleHudUpdate}
          onQuickRestart={handleQuickRestart}
        />
      </section>

      {/* BOTTOM COMPACT NAVIGATION BAR */}
      <NavigationBar
        activeScreen={activeScreen}
        onNavigate={handleNavigate}
        isCreatorMode={isCreatorMode}
        currentLevel={currentLevel}
        totalLevels={levels.length}
        onOpenLevels={() => setShowLevelSelect(true)}
        onOpenLeaderboard={() => setShowLeaderboard(true)}
        onOpenEditor={() => setShowEditor(true)}
        onOpenSettings={() => setShowSettings(true)}
        onQuickRestart={handleQuickRestart}
      />

      {/* MODALS */}
      {/* 1. Level Select Modal */}
      {showLevelSelect && (
        <LevelSelectModal
          levels={levels}
          currentLevelId={currentLevel.id}
          records={records}
          theme={currentTheme}
          isCreatorMode={isCreatorMode}
          onSelectLevel={handleSelectLevel}
          onDeleteLevel={handleDeleteLevel}
          onOpenEditor={() => {
            setShowLevelSelect(false);
            setShowEditor(true);
          }}
          onOpenLeaderboard={() => {
            setShowLevelSelect(false);
            setShowLeaderboard(true);
          }}
          onClose={() => setShowLevelSelect(false)}
        />
      )}

      {/* 2. Global & Level Leaderboard Modal */}
      {showLeaderboard && (
        <LeaderboardModal
          currentLevelId={currentLevel.id}
          levels={levels}
          theme={currentTheme}
          localRecords={records}
          playerName={playerName}
          telegramUser={tgUser}
          tgUser={tgUser}
          onUpdatePlayerName={handleUpdatePlayerName}
          onSelectLevel={handleSelectLevel}
          onClose={() => setShowLeaderboard(false)}
        />
      )}

      {/* 3. Settings Modal */}
      {showSettings && (
        <SettingsModal
          currentTheme={themeName}
          themeName={themeName}
          onSelectTheme={setThemeName}
          scanlines={scanlines}
          onToggleScanlines={() => setScanlines((v) => !v)}
          screenShake={screenShakeEnabled}
          onToggleScreenShake={() => setScreenShakeEnabled((v) => !v)}
          touchControlsMode={touchMode}
          touchMode={touchMode}
          onChangeTouchMode={setTouchMode}
          onSelectTouchMode={setTouchMode}
          isMuted={isMuted}
          isBgmPlaying={isBgmPlaying}
          onToggleMute={handleToggleMute}
          onToggleBgm={handleToggleBgm}
          sfxVolume={sfxVolume}
          onChangeSfxVolume={handleSfxVolume}
          bgmVolume={bgmVolume}
          onChangeBgmVolume={handleBgmVolume}
          playerName={playerName}
          onChangePlayerName={handleUpdatePlayerName}
          isCreatorMode={isCreatorMode}
          onToggleCreatorMode={handleToggleCreatorMode}
          onResetProgress={handleResetProgress}
          onClose={() => setShowSettings(false)}
        />
      )}

      {/* 4. Level Editor & Custom Map Creator */}
      {showEditor && (
        <LevelEditor
          levels={levels}
          initialLevel={currentLevel}
          theme={currentTheme}
          isCreatorMode={isCreatorMode}
          onToggleCreatorMode={handleToggleCreatorMode}
          onSaveLevel={handleSaveLevel}
          onResetLevel={handleResetLevel}
          onDeleteLevel={handleDeleteLevel}
          onPlaytest={handlePlaytest}
          onClose={() => setShowEditor(false)}
        />
      )}

      {/* 5. Embed Drawer (Iframe / Widget Generator) */}
      {showEmbedDrawer && (
        <EmbedDrawer
          currentLevelId={currentLevel.id}
          themeName={themeName}
          onClose={() => setShowEmbedDrawer(false)}
        />
      )}

      {/* 6. Victory & Rank Medal Modal */}
      {victoryData && (
        <VictoryModal
          data={victoryData}
          theme={currentTheme}
          onNextLevel={handleNextLevel}
          onRetryLevel={() => {
            setVictoryData(null);
            handleQuickRestart();
          }}
          onOpenLeaderboard={() => {
            setVictoryData(null);
            setShowLeaderboard(true);
          }}
          onShareToTelegram={() => {
            shareScoreToTelegram({
              levelName: victoryData.level.name,
              levelId: victoryData.level.id,
              timeStr: formatHundredths(victoryData.time),
              deaths: victoryData.deaths,
            });
          }}
          onClose={() => setVictoryData(null)}
        />
      )}
    </main>
  );
}
